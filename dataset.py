...
class TUMRGBDDataset(object):
    ...
    def __init__(self, path, register=True):
        ...
        self.depth = ImageReader(depth_ids, depth_timestamps)  # for baseline dataset(ex. rgbd_dataset_frieburg1_room)
...
###### Test Dataset #####
class TestImageReaderDepth(object):
    def __init__(self, ids, timestamps=None, cam=None):
        self.ids = ids
        self.timestamps = timestamps
        self.cam = cam
...
class TestImageReader(object):
    def __init__(self, ids, timestamps=None, cam=None):
        ...
        self.ahead = 0      # 10 images ahead of current index
        self.waiting = 1.5   # waiting time
...
class TestDataset(object):
    '''
    path example: 'test_dataset/'
    depth scale is 1 if you use depth map directly into depth camera
    '''
    cam = namedtuple('camera', 'fx fy cx cy scale')(
        381.694, 381.694, 323.56, 237.11, 1)
    def __init__(self, path, register=True):
        path = os.path.expanduser(path)
        print(path)

        if not register:
            rgb_ids = self.listdir(path, 'rgb')
            depth_ids = self.listdir(path, 'depth')
        else:
            rgb_imgs = self.listdir(path, 'rgb')
            depth_imgs = self.listdir(path, 'depth')
            
            
        self.rgb = TestImageReader(rgb_imgs, None)
        self.depth = TestImageReaderDepth(depth_imgs, None)
        self.timestamps = None
