...
class RGBDPTAM(object):
    def __init__(self, params):
        ...
        self.calib_dir = params.calib_dir
        self.lidar = params.lidar
    ...
    def save_pose(self, frame):
        pose = frame.pose.matrix()[:3]
        R = pose[:, :3]
        t = pose[:, 3:]
        print()
        print('----pose----')
        print('R:', R, 't:', t)
        fname = '{}.npy'.format(frame.idx)
        
        if self.lidar:
            lidar_calib_dir = self.calib_dir.replace('RGBD', 'LIDAR')
            if not os.path.exists(lidar_calib_dir):
                os.mkdir(lidar_calib_dir)
            np.save(lidar_calib_dir + fname, pose)
        
        np.save(self.calib_dir + fname, pose)
...
def main():
    ...
    from dataset import TUMRGBDDataset, ICLNUIMDataset, TestDataset
    ...
    elif 'test' in args.dataset.lower():
        dataset = TestDataset(args.path)

    params = Params()
    params.calib_dir = calib_dir
    ...
    for i in range(len(dataset))[:]:
        ...
        if dataset.timestamps is None:
            # timestamp = i / 20.
            timestamp = i / params.fps
        else:
            timestamp = dataset.timestamps[i]

        time_start = time.time()
        # extract feature points and compute keypoints and descriptors
        feature.extract()
        ...
        if not args.no_viz:
            viewer.update()
            print('all mappoints:', len(viewer.all_mappoints))
            print('')
            print('all colors:', len(viewer.all_colors))
    ...
    all_mappoints = viewer.all_mappoints
    all_colors = viewer.all_colors
    
    # create 3d point cloud
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(np.asarray(all_mappoints))
    pcd.colors = o3d.utility.Vector3dVector(np.asarray(all_colors))
    
    o3d.io.write_point_cloud('test_results/mesh.ply', pcd)
    
    print('save ok')

    # print('all mappoints:', all_mappoints)
    # print('')
...
def main_for_lidar():
    ...
    parser.add_argument('--dataset', type=str, default='test',
        help='dataset (TUM/ICL-NUIM/test)')
    # parser.add_argument('--path', type=str, help='dataset path',
    #     default='path/to/your/TUM_RGBD/rgbd_dataset_freiburg1_room')
    parser.add_argument('--path', type=str, help='dataset path',
        default='./rgbd_dataset/RGBD/TestDatasetBit16')
    args = parser.parse_args()
